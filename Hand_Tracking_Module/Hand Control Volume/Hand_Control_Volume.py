...
# Pycaw module to device speakers
...
minVol = volRange[0]
maxVol = volRange[1]
#print(volRange) # Volume range -95.25 - 0
...
while True:
    ...
    if len(lmList) != 0:
        ...
        #print(length) # Hand Range 20 - 170

        # Hand Range 20 - 170
        # Volume Range -95.25 - 0

        vol = np.interp(length, [20, 170], [minVol, maxVol])
        volBar = np.interp(length, [20, 170], [350, 150])
    ...
    cv2.putText(img, f'Vol: {int(volPer)} %', (20, 400), cv2.FONT_HERSHEY_PLAIN, 2,
                (255, 0, 0), 3)
